...
class NetStringToStructSimpleTest extends FunSuite{
  ...
  test("evaluate works for valid input") {
    assert(testedObject.evaluate("0.0.0.128/25").net == Integer.parseUnsignedInt("00000000000000000000000010000000", 2))
    assert(testedObject.evaluate("0.0.0.1/24").net ==   Integer.parseUnsignedInt("00000000000000000000000000000000", 2))
    assert(testedObject.evaluate("0.0.1.128/25").net == Integer.parseUnsignedInt("00000000000000000000000110000000", 2))
    assert(testedObject.evaluate("0.0.1.128/24").net == Integer.parseUnsignedInt("00000000000000000000000100000000", 2))
    assert(testedObject.evaluate("255.0.0.0/8").net ==  Integer.parseUnsignedInt("11111111000000000000000000000000", 2))
    assert(testedObject.evaluate("255.0.0.1/8").net ==  Integer.parseUnsignedInt("11111111000000000000000000000000", 2))
    assert(testedObject.evaluate("255.0.0.1/7").net ==  Integer.parseUnsignedInt("11111110000000000000000000000000", 2))
  }
  ...
  test("It does not fall on invalid input") {
    assert(testedObject.evaluate("123412312312").mask == 0)
    assert(testedObject.evaluate("").mask == 0)
  }
}
